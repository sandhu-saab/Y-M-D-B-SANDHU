...
from info import temp  # Make sure temp.U_NAME is available
...
async def send_msg(bot, filename, caption): 
    try:
        ...
        # Updated message format with IMDb link integrated into IMDB Info
        text = f"🎥 {filename} ({year if year else 'N/A'})\n\n"
        text += f"{language}\n\n"
        
        # IMDb info with clickable link
        ...
        if poster_url:
            # Send as spoiler with original IMDb image URL
